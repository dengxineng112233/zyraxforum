...
async function initRegisterPage() {
  ...
  function normalizeCode(v) { return String(v || "").split("").filter((ch) => ch >= "0" && ch <= "9").join("").slice(0, 6); }
  ...
  if (sendBtn) sendBtn.addEventListener("click", async () => {
    ...
    if (!email) return setMessage(msg, "\u5148\u586b\u5199\u90ae\u7bb1\u3002", "err");
    setMessage(msg, "\u6b63\u5728\u53d1\u9001\u9a8c\u8bc1\u7801...");
    try {
      const data = await api("/api/auth/send-register-code", { method: "POST", body: JSON.stringify({ email }) });
      setMessage(msg, data.devCode ? `\u9a8c\u8bc1\u7801\u5df2\u751f\u6210\uff1a${data.devCode}\uff08\u914d\u7f6e\u90ae\u4ef6\u670d\u52a1\u540e\u4f1a\u53d1\u5230\u90ae\u7bb1\uff09` : "\u9a8c\u8bc1\u7801\u5df2\u53d1\u9001\uff0c\u8bf7\u67e5\u770b\u90ae\u7bb1\u3002", "ok");
      startCooldown(60);
    } catch (err) {
      setMessage(msg, err.message, "err");
    }
  ...
  if (codeInput) codeInput.addEventListener("input", () => codeInput.value = normalizeCode(codeInput.value));
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const username = $("username").value.trim().split("").map((ch) => ch.trim() === "" ? "_" : ch).join("");
    const email = $("email").value.trim();
    const verificationCode = normalizeCode($("verificationCode").value);
    const password = $("password").value;
    const confirm = $("confirmPassword").value;
    if (verificationCode.length !== 6) return setMessage(msg, "\u8bf7\u8f93\u5165 6 \u4f4d\u90ae\u7bb1\u9a8c\u8bc1\u7801\u3002", "err");
    if (password !== confirm) return setMessage(msg, "\u4e24\u6b21\u5bc6\u7801\u4e0d\u4e00\u81f4\u3002", "err");
    setMessage(msg, "\u6ce8\u518c\u4e2d...");
    try {
      const data = await api("/api/auth/register", { method: "POST", body: JSON.stringify({ username, email, password, verificationCode }) });
      currentUser = data.user;
...
async function initAdminPage() {
  ...
  if (!currentUser || !(currentUser.role === "admin" || adminName === "admin" || adminName === "test")) {
    root.innerHTML = `<div class="empty">\u53ea\u6709 admin \u548c test \u8d26\u53f7\u53ef\u4ee5\u8fdb\u5165\u7ba1\u7406\u9762\u677f\u3002</div>`;
    return;
  }
  function adminTime(ts) {
    if (!ts) return "\u4ece\u672a\u4e0a\u7ebf";
...
async function initAdminKeyPage() {
  await loadSession();
  updateNav();
  const form = $("adminKeyForm");
  if (!form) return;
  form.onsubmit = async (e) => {
    e.preventDefault();
    const msg = $("authMsg");
    setMessage(msg, "\u6b63\u5728\u9a8c\u8bc1\u5bc6\u94a5...");
    try {
      await api("/api/admin/unlock", { method: "POST", body: JSON.stringify({ key: $("adminKey").value }) });
      setMessage(msg, "\u89e3\u9501\u6210\u529f\uff0c\u6b63\u5728\u8fdb\u5165\u540e\u53f0\u3002", "ok");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (page === "admin-key") await initAdminKeyPage();
});
