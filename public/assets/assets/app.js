...
async function loadPosts() {
  ...
  const sort = encodeURIComponent($("sortSelect")?.value || "new");
  const category = encodeURIComponent(activeCategory);
  const data = await api(`/api/posts?q=${q}&sort=${sort}&category=${category}`);
...
function updateNav() {
  ...
  if (compose) compose.style.display = currentUser ? "inline-flex" : "none";
...
function renderTopicListShell() {
  ...
  if (!root) return;
  root.innerHTML = `<div class="panel feed-tools"><div class="feed-tools-left"><div class="tabs"><button class="tab active" type="button">最新</button><button class="tab" type="button">热门</button><button class="tab" type="button">未读</button></div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><input class="input" id="searchInput" placeholder="搜索主题、内容或作者..." style="width:min(360px,70vw)"/><select class="select" id="sortSelect" style="width:142px"><option value="new">最新发布</option><option value="hot">最多点赞</option><option value="comment">最多评论</option></select></div></div><form class="panel composer locked" id="composer"><h2 class="section-title">发布新主题</h2><div class="field"><label for="postTitle">标题</label><input class="input" id="postTitle" maxlength="80" placeholder="写一个清晰标题" required /></div><div class="field" style="margin-top:12px"><label for="postCategory">板块</label><select class="select" id="postCategory"></select></div><div class="field" style="margin-top:12px"><label for="postBody">内容</label><textarea class="textarea" id="postBody" maxlength="3000" placeholder="分享问题、经验或公告..." required></textarea></div><button class="pill primary" type="submit" style="margin-top:12px">发布主题</button><div class="message" id="postMsg"></div></form><div class="panel topic-list" id="postList"></div>`;
...
function renderComposer() {
  ...
  composer.classList.toggle("locked", !currentUser);
...
async function refreshList() {
  try {
    ...
    const list = $("postList");
    if (list) list.innerHTML = `<div class="empty">API 未连接：${safeText(err.message)}<br>请用 <span class="kbd">npm run dev</span> 或部署到 Cloudflare Pages Functions 预览。</div>`;
  }
}
...
async function seedDemo() {
  ...
  const demo = [
    { title: "欢迎来到 CloudForum", category: "公告", body: "这是接入 Cloudflare D1 后的云端论坛。注册用户、帖子、评论、点赞都会存到 D1。" },
...
async function renderForum() {
  ...
  if (topicId) await renderTopicDetail(topicId);
  else {
    document.title = "CloudForum - 首页";
...
async function initForumPage() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (topic) {
      e.preventDefault();
      history.pushState(null, "", topic.getAttribute("href"));
      await renderForum();
    }
    if (home) {
      e.preventDefault();
